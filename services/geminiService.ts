...
// A function to get the initialized AI client.
// This ensures we use the latest API key set by the user.
const getAiClient = (): GoogleGenAI => {
    const apiKey = process.env.API_KEY;

    if (!apiKey) {
        throw new Error("Gemini API key not found. Please set it on the initial screen.");
    }
    
    // Create a new instance for each call to ensure it's always up-to-date.
...
export const getReviewContent = async (topics: string[]): Promise<string> => {
  try {
    ...
    return "Sorry, I couldn't fetch the review content at the moment. Please check your API key and the console for more details.";
  }
};
...
Provide a single, concise, and helpful hint to guide the student in the right direction. Do not give away the solution. Focus on the next logical step or a potential misunderstanding.`;
    ...
    return "Sorry, I couldn't generate a hint right now. Please check your API key and try again later.";
  }
};
...
Provide constructive feedback using markdown. Start with an overall assessment (e.g., "### Overall Assessment: Correct", "### Overall Assessment: Partially Correct", "### Overall Assessment: Needs Improvement"). Then, provide a "### Detailed Feedback" section explaining why, pointing out any errors, and suggesting improvements for correctness, efficiency, or code style.`;
    ...
    return "Sorry, I couldn't evaluate your answer at the moment. Please check your API key, your submission, and try again.";
  }
};
